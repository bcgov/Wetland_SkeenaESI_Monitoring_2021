...
WetlandsProcessed<-readRDS(file = 'tmp/AOI/WetlandsProcessed')
...
###  Write out Witset target wetlands
WitWets<-c('39670','39591','39847','8566','8557','8569','8685','8740')
Witset_KML <- SampleStrata_wFREP_CGL_Other_Geo %>%
  dplyr::filter(Wetland_Co %in% WitWets) %>%
  st_transform(crs="+proj=longlat +datum=WGS84") %>%
  as_Spatial() %>%
  writeOGR(dsn=file.path(dataOutDir,"Witset_WetlandSample.kml"), layer= "Witset_WetlandSample", driver="KML", overwrite_layer=TRUE)

###  Write out Witset target wetlands
WFNWets<-c('29339')
WFN_KML_2 <- SampleStrata_wFREP_CGL_Other_Geo %>%
  dplyr::filter(Wetland_Co %in% WFNWets) %>%
  st_transform(crs="+proj=longlat +datum=WGS84") %>%
  as_Spatial() %>%
  writeOGR(dsn=file.path(dataOutDir,"WFN_KML_2.kml"), layer= "WFN_KML_2", driver="KML", overwrite_layer=TRUE)
