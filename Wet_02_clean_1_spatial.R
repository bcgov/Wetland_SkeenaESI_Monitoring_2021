...
roads_keep <- roads_sf %>%
  ...
  filter(!COMMENTS %in% c("Deactivated","Private","Private Driveway", "Private Airstrip","Cleared, Not Built?","Over Grown",
                          "Mostly overgrown soon impassable","BRIDGE PULLED","BRIDGES PULLED OR WILL BE SHORTLY",
                          "Major culvert pull"))
...
write_sf(readRDS(file='tmp/AOI/roads_keep'), file.path(spatialOutDir,"roads_keep.gpkg"))
...
ESI_OW <-readRDS(file='tmp/ESI_OW') %>%
  ...
  st_union() %>%
  st_intersection(AOI)
saveRDS(ESI_OW, file='tmp/AOI/ESI_OW')

ESI_WFN <-readRDS(file='tmp/ESI_WFN') %>%
  st_set_crs(3005) %>%
  st_intersection(AOI)
saveRDS(ESI_WFN, file='tmp/AOI/ESI_WFN')

ESI_LBN <-readRDS(file='tmp/ESI_LBN') %>%
  st_set_crs(3005) %>%
  st_intersection(AOI)
saveRDS(ESI_LBN, file='tmp/AOI/ESI_LBN')

ESI_Gitxsan <-readRDS(file='tmp/ESI_Gitxsan') %>%
  st_set_crs(3005) %>%
  st_intersection(AOI)
saveRDS(ESI_Gitxsan, file='tmp/AOI/ESI_Gitxsan')

ESI_Gitxsan_wshd <-readRDS(file='tmp/ESI_Gitxsan_wshd') %>%
  st_set_crs(3005) %>%
  st_intersection(AOI)
saveRDS(ESI_Gitxsan_wshd, file='tmp/AOI/ESI_Gitxsan_wshd')

ESI_Gitanyow <-readRDS(file='tmp/ESI_Gitanyow') %>%
  st_set_crs(3005) %>%
  st_intersection(AOI)
saveRDS(ESI_Gitanyow, file='tmp/AOI/ESI_Gitanyow')
...
Moose <-readRDS(file='tmp/Moose') %>%
  st_intersection(AOI)
saveRDS(Moose, file='tmp/AOI/Moose')

WFN_AOI <-readRDS(file='tmp/WFN_AOI')
  #st_buffer(dist=0) %>%
  #st_intersection(AOI)
saveRDS(WFN_AOI, file='tmp/AOI/WFN_AOI')
